...
ggplot(Category, aes(y=as.factor(Var1), x=Freq, fill=Var2)) + geom_histogram(stat="identity") +
  xlab("Number of method") + ylab("Framework") + theme_light() + theme(axis.text=element_text(size=16), axis.title = element_text(size=28))


```
```{r}
class_alg <- deconvolution_review[, c("Method.name","Category")]
class_alg

result_list <- lapply(unique(class_alg$Category), function(class) {
  class_alg$Method.name[class_alg$Category == class]
})

names(result_list) = unique(class_alg$Category)
...
### In-depth investigation of framework

#### Regression
```{r}
View(deconvolution_review[deconvolution_review$Category == "Regression",])


View(deconvolution_review[deconvolution_review$Category == "Bayesian",])

View(deconvolution_review[deconvolution_review$Category == "Deep learning",])

View(deconvolution_review[deconvolution_review$Category == "Matrix factorization",])

View(deconvolution_review[deconvolution_review$Category == "Topic modeling",])

View(deconvolution_review[deconvolution_review$Category == "Deep learning",])

View(deconvolution_review[deconvolution_review$Category == "Optimal transport",])

View(deconvolution_review[deconvolution_review$Category == "Graph convolutional network",])
...
input_data <- deconvolution_review[,c("ST.counts", "ST.coordinates", "Image", "Reference.based...Reference.free")]
# To simplify, I remove the lines where I have optional written 

input_data <-input_data[-c(2,7,26,31,35),]

input_data$group <- paste(input_data$ST.counts, input_data$ST.coordinates, input_data$Image,input_data$Reference.based...Reference.free , sep="_")
...
sets <- list(
  Use_Counts = which(input_data$ST.counts == "Yes"),
  Use_Coord = which(input_data$ST.coordinates == "Yes"),
...
ggvenn(sets, c("Reference_based","Use_Coord", "Use_Image"), show_percentage = F, fill_color = colors_pal[c(3,1,2)], text_size=10, set_name_size=5) 
ggsave("venn_diagram_methods_ref_based.svg", width=15, height=10)

ggvenn(sets, c("Reference_free","Use_Coord", "Use_Image"), show_percentage = F, fill_color = colors_pal[c(4,1,2)], text_size=10, set_name_size=5) 
ggsave("venn_diagram_methods_ref_free.svg", width=15, height=10)
...
```{r}
img <- deconvolution_review[,c("Method_name", "Image", "Image_int")]

# filter out yes
img_yes <- img[img$Image %in% c("Yes", "Optional"),]
View(img_yes)
```
...
## Platform effect
```{r}
deconvolution_review$Method_name[deconvolution_review$Batch.platform.correction == "Platform effect"]
```
